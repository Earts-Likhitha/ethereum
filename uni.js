...
const uniContractAddress ='0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984'//uni contract address
...
async function sendUNITransaction(mnemonic, amountInUNI, recipientAddress) {
  try {
    ...
    const uniContract = new web3.eth.Contract(uniContractABI, uniContractAddress);

    const balanceInWei = await uniContract.methods.balanceOf(senderAccount.address).call();

    // Convert the balance from Wei to UNI (considering  decimals)
    const balanceInUNI = web3.utils.fromWei(balanceInWei, 'ether');

    console.log(`UNI Balance for Address ${senderAccount.address}: ${balanceInUNI} UNI`);
    
    const amountInWei = web3.utils.toWei(amountInUNI.toString(), 'ether');
    const transferData = uniContract.methods.transfer(recipientAddress, amountInWei).encodeABI();
...
sendUNITransaction("light shuffle sword will rude muscle pepper order symbol conduct bomb card","0.001", "0xd87D25a7e3Ea78bB39c3A27EF20c5c918E2bf0e8");
